...
type Correlation struct {
	N        int
	R        *float64
	Enough   bool   // MinCorrelationSamples(90) 以上か
}
```

相関係数は3点あれば計算できてしまう。**計算できることと、信じてよいことは別。**

値を隠すと「まだ見られない」ことにも気づけないので、
**値は出したうえで `enough: false` を付ける**。
判断に使ってよいかどうかを、読み手ではなく出す側が明示する。

n=5 で r=+0.657 のような数字は、出し方を間違えると行動を変える根拠にされる。
