...
| | 何をするか | どこで設定するか |
|---|---|---|
| **1. サインアップを閉じる** | 新規登録そのものを止める | Supabase → Authentication → Providers |
| **2. `sub` の許可リスト** | 許可した `auth.users.id` 以外を 401 にする | Cloud Run の環境変数 `ALLOWED_USER_IDS` |

**2 だけでも穴は塞がる。** 1 は Supabase のダッシュボード操作でリポジトリに残らないため、
設定を戻したときや別プロジェクトに移したときに静かに開く。2 はコードとして残る。
...
**未設定だと起動ログに WARN が出る。** 出ていたら塞がっていない。
