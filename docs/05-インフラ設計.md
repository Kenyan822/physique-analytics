...
**スキーマを先に、コードを後に。** `deploy-api` は `main` への push で
自動的に走るので、マイグレーションを含む PR をそのままマージすると
「テーブルが無いのに新しいコードが動いている」時間ができる。

`migrate-db` は `workflow_dispatch` で**実行するブランチを選べる**
（`actions/checkout` が起動元の ref を使う）。なので次の順序にする。

1. **PR のブランチを指定して `migrate-db` を `up` で実行する**
2. マージする（`deploy-api` が走り、既にテーブルがある状態にコードが載る）
3. `migrate-db` を `version` で確認する

逆順でも致命的ではない（新しいエンドポイントが 500 を返すだけで、
既存の機能とヘルスチェックは動き続ける）が、窓を作る理由が無い。

#### 適用前に確かめること

**使い捨ての DB で往復を確かめる。** 戻せないマイグレーションを本番に
当てると、失敗したときに手で直すことになる。
