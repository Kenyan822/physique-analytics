...
| プレビュー | PR ごとに自動生成 |
| Deployment Protection | **無効** |

> **いまは GitHub リポジトリが接続されていない**（[#173](https://github.com/Kenyan822/physique-analytics/issues/173)）。
> 上の表は接続後の姿で、現状は production も プレビューも自動では出ない。
> マージしても本番は古いままなので、反映には `vercel deploy --prod` を手で打つ必要がある。
> 接続はダッシュボード操作なので CLI からはできない。

**Deployment Protection を切っている。** 既定では `*.vercel.app` に Vercel の
ログインが要り、アプリのログインと合わせて二重になる。ジムでスマホから使うのに
毎回2回ログインするのは現実的でない。守りはアプリ側にある。

| 層 | 何で守るか |
|---|---|
| 画面 | 未ログインは `/login` へ（`proxy.ts`） |
| API | `ALLOWED_USER_IDS` に無い `sub` は 401（未設定なら全員拒否） |
| DB | RLS 有効・ポリシー無し。PostgREST から読めない（`000012`） |

#### 環境変数

| 変数 | 値の性質 |
|---|---|
| `API_BASE_URL` | Cloud Run の URL。**サーバ側でだけ使う** |
| `NEXT_PUBLIC_SUPABASE_URL` | 公開前提（ブラウザに載る） |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | 公開前提。単体では何も読めない（RLS で閉じている） |

#### `.vercelignore`

**`private/` を絶対に上げない**（[ADR-0002](adr/0002-separate-personal-data.md)）。`.gitignore` にも入れてあるが、
除外が1箇所だけだと片方を直したときに気づけないので、ここでも明示する。

**Web 以外のディレクトリはパターンの先頭にスラッシュを付ける。** 付けないと
どの階層にも一致し、`api/` が `web/lib/api/` まで巻き込んでビルドが落ちる（実際に踏んだ）。
...
**適用は `deploy-api` の前段ゲートで自動実行する**（[ADR-0016](adr/0016-auto-migrate-on-deploy.md)）。

```
main に push
   ↓
[ migrate ジョブ ]  ← ここが落ちたら止まる。本番のコードは無傷
   ↓  needs
[ deploy ジョブ ]
```

**スキーマは常にコードより先に進める。** コードは traffic 切り替えで数秒で戻せるが、スキーマは戻せない。前段ゲートにすることで、この順序が人の記憶ではなく構造で守られる。

**引き受ける制約: マイグレーションは追加のみ（expand/contract）。** migrate → deploy の順なので、新しいスキーマを旧コードが読む瞬間が必ずできる。列の削除・リネーム・`not null` 化は3回のデプロイに分ける（追加 → 切り替え → 削除）。

**自動で流すのは `up` だけ。** `down` は戻せない変更を無人で走らせないため手動に残す。

手動の操作は **GitHub Actions の `migrate-db` ワークフロー**（`workflow_dispatch`）から行う。ローカルから流さないのは、接続文字列を手元に平文で置く必要がなくなるため。

| 用途 | command |
|---|---|
| 今のバージョンを確認する | `version` |
| **戻す** | `down` ＋ steps |
| dirty からの復旧・手動での再適用 | `up` |

```
Actions → migrate-db → Run workflow → command: version
```

#### 適用の順序

**マージすれば正しい順序で流れる。** `deploy-api` の `migrate` ジョブが
`deploy` の前段ゲートになっているため、手で順序を作る必要は無い
（[ADR-0016](adr/0016-auto-migrate-on-deploy.md)）。

```
マージ → migrate（up）→ deploy → スモークテスト
          ↑ 落ちたらここで止まる
```

**マイグレーションを書くときの制約は残る。** 追加のみにすること。
migrate → deploy の順なので「新しいスキーマ + 旧コード」が動く瞬間が必ずでき、
列の削除・リネームを1回でやるとその瞬間に旧コードが落ちる。

| やりたいこと | 何回に分けるか |
|---|---|
| 列を足す | 1回 |
| 列を消す・リネームする | **3回**（追加 → コードを切り替え → 削除） |
| `not null` にする | **2回**（先に既存行を埋める → 制約を付ける） |

#### 適用前に確かめること

**使い捨ての DB で往復を確かめる。** 自動適用になった分、ここが最後の砦になる。
戻せないマイグレーションを本番に当てると、失敗したときに手で直すことになる。

```bash
docker compose exec -T db psql -U physique -d postgres -c "create database migrate_check"
# up → down → up が通ることを見る
```
...
| **記録の長期保管** | **CSV エクスポート** | **月次** | **Cloud Scheduler → Cloud Run Jobs（自動）** |
| **計画の設定** | **JSON エクスポート**（`/transfer`） | 変更したとき | 手動 |
| `private/` 全体 | iCloud Drive 等で同期 | 常時 | — |

**設定は CSV に含まれない。** フェーズ・栄養パラメータ・部位別 MEV/MRV・月次目標の起点・
計画ブロックは DB が正なので（[ADR-0015](adr/0015-plan-settings-in-db.md)）、記録とは別に
保管する。入れ子の構造を持つため CSV ではなく JSON。

`/transfer` の「計画の設定」から書き出し・読み込みができる。**新しい API は足していない** ——
既存の `GET/PUT /v1/plan` と `GET/PUT /v1/plan/blocks` を組み合わせているだけ。

読み込むと**設定はまるごと置き換わる**。フェーズの `id` は振り直されるが、どの表からも
参照されていないので影響は無い。
...
- R2 は RLS が使えないため、**API が所有者を検証してから署名付きURLを発行する**（[ADR-0008](adr/0008-r2-photo-storage.md)）

### 8.0 入口は2つある

**Go API だけが DB への入口ではない。** Supabase は Postgres の前に PostgREST を
自動で立てる。

```
        ┌─ Cloud Run（Go API）  JWT を検証する
Postgres┤
        └─ PostgREST           **Supabase が自動で立てる。Go API を通らない**
```

PostgREST は既定で `public` スキーマを公開し、匿名ロール `anon` でアクセスできる。
anon key はブラウザの JS に入る前提の公開情報なので、**隠して守ることはできない**。

`000012_lock_down_public_schema` で塞いである。

| | 何をしたか |
|---|---|
| RLS | 全テーブルで有効化。**ポリシーは作らない**（＝素通りできないロールには行が無く見える） |
| 権限 | `anon` / `authenticated` から剥奪 |
| 既定権限 | **今後作るテーブルにも自動で付かないようにした**。これが無いと次のマイグレーションでまた開く |

アプリは影響を受けない。`postgres` は `rolbypassrls = true` で、テーブルの所有者でもある。

**#51（サインアップ）と `ALLOWED_USER_IDS` はどちらも Go API の入口にしか効かない。**
この2つを完璧にしても PostgREST 側は閉じない。

### 8.1 所有者の絞り込み（現状）

**DB のテーブルに `user_id` 列はまだ無い。** 1人で使う前提のため、
[#51](https://github.com/Kenyan822/physique-analytics/issues/51) で「まずサインアップを閉じる」と決めている。
`user_id` + RLS に移るのは「他人にも使わせる」と決めたとき。

そのため、**JWT の検証を通っただけでは所有者を区別できない。**
Supabase のサインアップが開いていれば、登録した人は誰でも全データを読める。
守りは2重にする。

| | 何をするか | どこで設定するか |
|---|---|---|
| **1. `sub` の許可リスト** | 許可した `auth.users.id` **以外を 401 にする** | Cloud Run の環境変数 `ALLOWED_USER_IDS` |
| 2. サインアップを閉じる | 新規登録そのものを止める | Supabase → Authentication → Providers |

**1 だけで穴は塞がる。** 2 はダッシュボード操作でリポジトリに残らないため、
設定を戻したときや別プロジェクトに移したときに静かに開く。1 はコードとして残る。

**未設定なら全員拒否する**（fail-closed。[#152](https://github.com/Kenyan822/physique-analytics/issues/152)）。
「設定するまで開いている」は「ログインするまで開いている」と同じで、順序が逆になるため。

```bash
# 自分の auth.users.id を調べる（Supabase の SQL Editor）
select id, email from auth.users;

# Cloud Run に入れる。複数人ならカンマ区切り
ALLOWED_USER_IDS=11111111-1111-1111-1111-111111111111
```

**設定するまで `/health` 以外はすべて 401 になる。** 起動ログに ERROR が出るので気づける。
`/health` を公開のままにしてあるのは、デプロイのスモークテストと keepalive が叩くため
（ここも閉じるとデプロイが通らない）。

シークレットではないので Secret Manager には入れない（UUID であって鍵ではない）。

**順序**: デプロイ → ログイン（401 になる）→ `auth.users` から id を取る → 設定 → 使える。
初回だけこの往復が要る。
