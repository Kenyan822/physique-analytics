...
### Description は LLM への唯一の入力 —— スキーマを足したら更新する

`query` ツールの Description に使えるテーブルを列挙しているが、**ここに書いていない
テーブルは事実上使われない。** LLM は DB を覗けないので、この文字列が世界のすべてになる。

マイグレーションで `meals` / `daily_metrics` / `contests` / `blood_tests` などを足したのに
Description が5テーブルのままだった時期があり、その間これらは `query` の対象外だった。
**エラーも警告も出ない。**「そのテーブルは無い」と LLM が判断して別の答えを返すだけ。

```go
// 足したらここも足す、とコメントで縛っておく
// **ここが LLM への唯一の入力。** 書いていないテーブルは事実上使われないので、
// マイグレーションでテーブルを足したらここも足す
Description: "... テーブル —— トレーニング: exercises / ... 検査: blood_tests / blood_test_items。",
```

**判断**: 自動生成（`information_schema` から組み立てる）も考えたが、やらなかった。
列挙するだけでは足りず「どう解釈するか」（論理削除・JST の日付）を添える必要があり、
そこは手で書くしかない。自動生成すると、この文脈が落ちる。

型の生成（`openapi.yaml` → Go）と違い、**Description は仕様ではなく説明**なので、
自動化の対象として筋が悪い。
