...
**どこから叩いてもよい**（リポジトリルートからの `./ios/scripts/device.sh` も可）。

> **シェルの罠。** `$0` は相対パスなので、`cd` したあとに `dirname "$0"` を
> 評価すると別の場所を指す。**`cd` の前に絶対パスで捕まえる。**
>
> ```bash
> HERE="$(cd "$(dirname "$0")" && pwd)"    # ← cd より前
> cd "$HERE/.."
> ```
>
> ルートから叩くと `ios/ios/scripts/` を探して落ちていた（#213）。
> 出るメッセージが「iPhone が見つからない」だったので、繋ぎ忘れと区別がつかなかった。
