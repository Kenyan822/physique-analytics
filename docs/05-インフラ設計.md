...
**対策: GitHub Actions の cron で定期的に疎通させる。**
...
jobs:
  ping:
    ...
    steps:
      - run: curl -fsS "${{ secrets.API_HEALTHZ_URL }}"
```

ヘルスチェックが DB 接続を含むため、これだけで非アクティブ判定を回避できる。**週1回で十分**（閾値は7日）。
...
| 環境 | 用途 | DB | 実行 |
|---|---|---|---|
| local | 開発 | **Docker Compose の Postgres** | `go run` |
...
Transaction mode ではプリペアドステートメントが使えないため、**pgx の `QueryExecModeSimpleProtocol` を指定する**。
...
実行はローカルから行う（Supabase は接続文字列でアクセスでき、IP 制限を設けていないため）。

```bash
migrate -path api/migrations -database "$SUPABASE_DIRECT_URL" up
```

**マイグレーションは直接接続（ポート5432）を使う。** Transaction mode ではセッションをまたぐ DDL が扱いにくいため。
...
| 死活監視 | `/healthz` | DB 接続を含む疎通確認。§2.1 の keepalive も兼ねる |
